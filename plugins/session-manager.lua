...
local function retrieve_workspace_data(window, workspace_name)
...
local function save_to_json_file(data, file_path)
	...
	local success, dirname = wezterm.run_child_process({ "dirname", file_path })
	if not success then
		return false
	end
	dirname = utils.trim(dirname)
	local success = wezterm.run_child_process({ "mkdir", "-p", dirname })
	if not success then
		return false
	end
...
local function recreate_workspace(window, workspace_data)
	...
	window.mux_window:set_workspace(workspace_data.name)
	...
	if
		...
		or foreground_process:find("fish")
	then
		-- Safe to close
...
function session_manager.restore_state(window)
	local folder_path = wezterm.home_dir .. "/.config/wezterm/sessions/"

	local success, stdout, stderr = wezterm.run_child_process({ "ls", folder_path })
	if not success then
	...
	for file in stdout:gmatch("[^\r\n]+") do
		...
		if not workspace_data then
			window:toast_notification("WezTerm", "Workspace state file not found for: " .. file, nil, 4000)
			return
		end
		local workspace_name = workspace_data.name
	...
	window:perform_action(act.SwitchToWorkspace({ name = "default" }), window.active_pane())
end
...
function session_manager.save_state(window)
	for _, workspace_name in ipairs(wezterm.mux.get_workspace_names()) do
		local data = retrieve_workspace_data(window, workspace_name)
